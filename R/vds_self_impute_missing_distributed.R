...
source('./vds_impute.R')
couch.check.is.raw.imputed <- function(year,vdsid,db='vdsdata%2ftracking'){
  r <- rcouchtuils::couch.check.state(year,vdsid,'vdsraw_chain_lengths',db=db)
  result <- (r[1] != 'todo' )
  return (result)
}
...
process.yearly.files <- function(year,
                                 seconds=3600,
                                 base.dir="/data/pems/breakup",
                                 goodfactor=2,
                                 reconsider=FALSE,
  ...
  for( f in files){
    ...
    if(couch.check.is.raw.imputed(year,vds.id)){
      ...
      done <- self.agg.impute.VDS.site.no.plots(fname,f,path,year,
                                                seconds=seconds,
      ...
      if(done == 1){
        ...
        quit(save='no',status=10)
          ...
          rcouchtuils::couch.set.state(year,
                                       id=vds.id,
                                       doc=list('vdsraw_chain_lengths'=done),
                                       db=trackingdb)
      }
    }
  }
